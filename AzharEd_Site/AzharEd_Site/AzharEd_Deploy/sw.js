...
self.addEventListener('fetch',e=>{
  ...
  e.respondWith((async()=>{
    ...
    if(isContent){
      ...
      const net=fetch(req).then(res=>{if(res&&res.ok)c.put(url.href,res.clone());return res}).catch(()=>clean(hit));
      return clean(hit)||net;
    }
    try{
